...
//
contract VestingSeedSaleproxy is RoleControl {
    address public vestingSeedSaleaddress;

    constructor(address _VestingSeedSaleaddress) RoleControl() {
        require(
            _VestingSeedSaleaddress != address(0),
            "Zero address given for vesting"
        );
        vestingSeedSaleaddress = _VestingSeedSaleaddress;
    ...
    function updateContractAddress(address child) external onlyOwner {
        ...
        vestingSeedSaleaddress = child;
    }

    /* -------------------------------------------------------------------------- */
    /*                                  Fallback                                  */
    /* -------------------------------------------------------------------------- */
    //call back to actual contract vestingSeedSale
    ...
    fallback() external {
        _delegate(vestingSeedSaleaddress);
    }
}
